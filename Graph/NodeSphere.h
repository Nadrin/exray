...
#ifndef __NODE_SPHERE_H
#define __NODE_SPHERE_H

namespace exRay {

/// Obiekt sceny: Kula.
class NodeSphere : public Object
{
private: // cache
	float	cRadius;
	float	cSqRadius;
	...
	vector3	cCentre;
public:
	NodeSphere(const std::string &name, Node *parent);
	virtual ~NodeSphere(void);
...
/// Kreator klasy NodeSphere.
class NodeSphereCreator : public NodeCreator
{
public:
	virtual Node* operator()(const std::string &name, Node *parent=NULL) const
	{ return new NodeSphere(name, parent); }
};
